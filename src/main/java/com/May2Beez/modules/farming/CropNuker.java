...
public class CropNuker extends Module {
    ...
    public CropNuker() {
        super("Crop Nuker", new KeyBinding("Crop Nuker", Keyboard.KEY_Z, May2BeezQoL.MODID + " - Farming"));
    }

    @SubscribeEvent
    public void onTick(TickEvent.PlayerTickEvent event) {
        if(May2BeezQoL.config.farmingSpeedIndex == 0 && event.phase == TickEvent.Phase.END) return;
    ...
    private BlockPos closestCrop() {
        ...
        double r = May2BeezQoL.config.farmingRadius;
        BlockPos playerPos = Minecraft.getMinecraft().thePlayer.getPosition();
        playerPos = playerPos.add(0, 1, 0);
        Vec3 playerVec = Minecraft.getMinecraft().thePlayer.getPositionVector();
        Vec3i vec3i = new Vec3i(r, r, r);
        if (May2BeezQoL.config.farmShapeIndex == 1) {
            vec3i = new Vec3i(r, May2BeezQoL.config.downUpRange, r);
        }
        Vec3i vec3iCane = new Vec3i(r, 0, r);
        ArrayList<Vec3> warts = new ArrayList<>();
        if (playerPos != null) {
            switch (May2BeezQoL.config.farmNukeIndex) {
                case 0:
