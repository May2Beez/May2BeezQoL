...
public class FuelFilling {
    ...
    public void onTick(TickEvent event) {
        ...
        ArrayList<String> itemLore = SkyblockUtils.getItemLore(drill);
    ...
    public void onTickSecond(TickEvent.ClientTickEvent event) {
        ...
        if (!(fuel != -1 && fuel < May2BeezQoL.config.fuelThreshold)) return;
        ...
        switch (currentState) {
            ...
            case WAITING: {
                ...
                if (abiphoneIndex == -1) {
                    ...
                    currentState = states.NONE;
                    May2BeezQoL.config.refuelWithAbiphone = false;
                    Reset();
                    return;
                }
            ...
            case CALLING: {
                if (mc.thePlayer.openContainer instanceof ContainerChest) {
                    ...
                    LogUtils.addMessage("Fuel Refuelling - You don't have Jotraeline in contacts!", EnumChatFormatting.RED);
                    LogUtils.addMessage("Fuel Refuelling - Add the npc to abiphone and re-enable the feature in options menu", EnumChatFormatting.RED);
                    May2BeezQoL.config.refuelWithAbiphone = false;
                    Reset();
                }
                break;
            ...
            case REFILLING_DRILL: {
                if (mc.thePlayer.openContainer instanceof ContainerChest) {
                    ...
                    if (drillAnvil.getSlot(drillAnvil.inventorySlots.size() - 9 + drillSlotIndex) != null && drillAnvil.getSlot(drillAnvil.inventorySlots.size() - 9 + drillSlotIndex).getStack() == null) {
                        LogUtils.addMessage("Fuel Refuelling - Drill slot is empty!", EnumChatFormatting.RED);
                        return;
                    }
            ...
            case REFILLING_FUEL: {
                ...
                if (mc.thePlayer.openContainer instanceof ContainerChest) {
                    ...
                    switch (May2BeezQoL.config.fuelType) {
                        case 0: {
                            fuelName = "goblin egg";
                            break;
                        }
                    ...
                    LogUtils.addMessage("Fuel Refuelling - You have no fuel in inventory!", EnumChatFormatting.RED);
                    LogUtils.addMessage("Fuel Refuelling - Buy more fuel and re-enable the feature in options menu", EnumChatFormatting.RED);
                    May2BeezQoL.config.refuelWithAbiphone = false;
                    Reset();
                }
            }
