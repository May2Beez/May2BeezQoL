...
public class FishingMacro extends Module {
    ...
    public FishingMacro() {
        super("Fishing Macro", new KeyBinding("Fishing Macro", Keyboard.KEY_NONE, SkyblockMod.MODID + " - Player"));
    }
    ...
    public void onEnable() {
        ...
        KeyBinding.setKeyBindState(mc.gameSettings.keyBindSneak.getKeyCode(), SkyblockMod.config.sneakWhileFishing);
    ...
    public void onDisable() {
        super.onDisable();
        if (SkyblockMod.config.sneakWhileFishing) {
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        if (SkyblockMod.config.antiAfk && ++ticks > (30 + new Random().nextInt(30))) {
            ticks = 0;
        ...
        if (target != null) {
            if (SkyblockUtils.getMobHp(targetStand) <= 0) {
                target = null;
                mc.thePlayer.inventory.currentItem = SkyblockMod.config.rodSlot - 1;
            }
        }
        ...
        if (SkyblockMod.config.prioritizeSCs) {
    ...
    public void onRenderTick(RenderWorldLastEvent event) {
        ...
        if (target != null && attackDelay.hasReached(150)) {
            ...
            mc.thePlayer.inventory.currentItem = SkyblockMod.config.weaponSlot - 1;
            switch (SkyblockMod.config.weaponAttackMode) {
                case 0: {
                    click();
                    break;
                }
    ...
    private void findAndSetCurrentSeaCreature() {
        int ranga = SkyblockMod.config.scScanRange;
        ...
        if (filtered.isPresent()) {
            ...
            if (target != null && SkyblockUtils.getMobHp(targetStand) > 0) {
                ...
                if (SkyblockMod.config.lookDownWhenAttacking) {
                    RotationUtils.smoothLook(new RotationUtils.Rotation(0, mc.thePlayer.rotationYaw), 5);
                } else {
                    RotationUtils.smoothLook(RotationUtils.getRotationToEntity(target), 3);
                }
