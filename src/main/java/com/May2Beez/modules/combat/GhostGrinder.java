...
public class GhostGrinder extends Module {
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        if (lastClick-- <= 0) {
            if (dist < 5) {
                click();
                lastClick = SkyblockMod.config.clickDelay;
            }
        }
    ...
    public void onRenderWorld(RenderWorldLastEvent event) {
        ...
        RenderUtils.drawEntityBox(creeper, Color.orange, SkyblockMod.config.lineWidth, event.partialTicks);
    }
    ...
    private static Entity getClosestCreeper() {
        ...
        for (Entity entity1 : (Minecraft.getMinecraft().theWorld.loadedEntityList)) {
            if (entity1 instanceof EntityCreeper && !(((EntityCreeper) entity1).getHealth() == 0)) {
                ...
                if (dist < closest) {
                    if(SkyblockMod.config.radius != 0 && dist < SkyblockMod.config.radius) {
                        closest = dist;
                        eman = entity1;
                    }
                    if(SkyblockMod.config.radius == 0) {
                        closest = dist;
                        eman = entity1;
                    }
                }
