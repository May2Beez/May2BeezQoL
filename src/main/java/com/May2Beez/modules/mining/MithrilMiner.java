        ...
        switch (currentState) {
            ...
            case MINING: {
                ...
                if (!RotationUtils.running)
                    RotationUtils.smoothLook(RotationUtils.vec3ToRotation(target.getRandomVisibilityLine()), SkyblockMod.config.cameraSpeed);
                ...
                if (stuckTimer.hasReached(SkyblockMod.config.maxBreakTime) && !RotationUtils.running) {
                    KeyBinding.setKeyBindState(mc.gameSettings.keyBindAttack.getKeyCode(), false);
                    SkyblockUtils.SendInfo("Stuck for " + SkyblockMod.config.maxBreakTime + " ms, restarting.", false, name);
                    stuckTimer.reset();
                    currentState = State.SEARCHING;
                    searchingTimer.reset();
